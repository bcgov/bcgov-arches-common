        ...
        for node_alias in node_aliases:
            ...
            if value:
                return_value += self.format_value(
                    graph_lookup.get_node(node_alias).name, value, True
                )

        return return_value

    def format_value(
        self, name, value, show_name=True, value_connector=", ", unique_list=True
    ):
        ...
        if value is None or value == "" or type(value) is list and len(value) == 0:
            ...
            return self.display_pattern % (
                name,
                value_connector.join(value) if type(value) is list else value,
            )
