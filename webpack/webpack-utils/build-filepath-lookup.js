...
function buildFilepathLookup(path, staticUrlPrefix) {
    ...
    const getFileList = function (dirPath) {
        return fs
            ...
            .reduce((fileList, entries) => {
                const childPath = Path.join(dirPath, entries.name).replace(
                    /\\/g,
                    '/',
                );
        ...
        if (extension === 'js') {
            lookup[
                file
                    .replace(path, '')
                    .replace(/\\/g, '/')
                    .replace(extensionReplacementRegex, '')
                    .replace(/^\//, '')
            ] = {
                import: file,
                filename: `${prefix}/[name].[contenthash].${extension}`,
            };
        } else if (extension === 'css' || extension === 'scss') {
            lookup[
                Path.join('css', file.replace(path, ''))
                    .replace(/\\/g, '/')
                    .replace(extensionReplacementRegex, '')
                    .replace(/^\//, '')
            ] = {
