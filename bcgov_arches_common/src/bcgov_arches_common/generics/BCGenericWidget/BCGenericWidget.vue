...
import { mergeCardXNodeXWidgetData } from '@/bcgov_arches_common/generics/util.ts';
...
const {
    ...
    configOverrides?: AllowableWidgetOverrides;
    graphSlug: string;
    isDirty?: boolean;
    mode: WidgetMode;
    nodeAlias: string;
...
watchEffect(async () => {
    ...
    try {
        ...
        if (configOverrides && resolvedCardXNodeXWidgetData.value) {
            resolvedCardXNodeXWidgetData.value = mergeCardXNodeXWidgetData(
