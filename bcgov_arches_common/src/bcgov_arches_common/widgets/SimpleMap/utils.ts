...
export function buildLayersForFeature(
    ...
    const geomTypes: string[] = [
        ...new Set(
            featureArray.map(
                (f: Feature) =>
                    (f.geometry.type as string) ?? ('Geometry' as string),
            ),
        ),
    ] as string[];

    const hasPointLike =
        geomTypes.findIndex((geomType: string) =>
            /^(Point|MultiPoint)$/i.test(geomType),
        ) >= 0;
    // const hasPointLike = geomTypes.includes((geomType as string) => (/^(Point|MultiPoint)$/i.test(geomType));

    // const isLineLike = /^(LineString|MultiLineString)$/i.test(geomType);
    const hasLineLike =
        geomTypes.findIndex((geomType: string) =>
            /^(LineString|MultiLineString)$/i.test(geomType),
        ) >= 0;

    // const isPolygonLike = /^(Polygon|MultiPolygon)$/i.test(geomType);
    const hasPolygonLike =
        geomTypes.findIndex((geomType: string) =>
            /^(Polygon|MultiPolygon)$/i.test(geomType),
        ) >= 0;
    ...
    if (!hasPointLike && !hasLineLike && !hasPolygonLike) {
        layers.push({
            id: `${baseId}-fallback`,
            type: 'line',
            source: src,
...
export function removeLayersUsingSource(
    ...
    // Optionally remove the source after all layers are gone
