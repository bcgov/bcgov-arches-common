...
class GraphLookup:
    ...
    def __init__(self, graph_slug, node_aliases):
        ...
        self._node_aliases = (
            node_aliases.values() if isinstance(node_aliases, dict) else node_aliases
        )
        ...
        if not self._datatype_factory:
            ...
            for alias in self._node_aliases:
                ...
                if node:
                    ...
                    self._datatypes[alias] = self._datatype_factory.get_instance(
                        node.datatype
                    )
    ...
    def get_node(self, node_alias):
        ...
        if node_alias not in self._nodes:
            raise KeyError(
                "Graph %s not configured for node %s" % (self._graph_slug, node_alias)
            )
