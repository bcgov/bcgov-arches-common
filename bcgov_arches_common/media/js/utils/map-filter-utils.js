...
var mapFilterUtils = {
    WFS_URL:
        arches.urls.root +
        "bctileserver/geo/ows?service=WFS&version=1.0.0&request=GetFeature&typeNames={layer}" +
    ...
    isArchesGeometry: function (feature) {
        return (
            feature.properties &&
            ko.unwrap(feature.properties.resourceinstanceid)
        );
    },
    getWfsUrl: function (objectId, layer) {
        return this.WFS_URL.replace("{layer}", layer).replace(
            "{object_id}",
            objectId,
        );
    },
