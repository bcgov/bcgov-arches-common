...
const viewModel = BaseFilter.extend({
    initialize: function (options) {
            ...
            try {
                ...
                if (geoJSON.features.length > 1) {
                    hint.push({
                        level: "warning",
                        message:
                            "Only one feature is allowed for search filtering.  Ignorning all all but the first feature.",
                    });
                }
                ...
                if (!!feature.properties && !!feature.properties.buffer) {
                    ...
                    try {
                        ...
                        hint.push({
                            level: "warning",
                            message:
                                "Buffer must be an integer between 0 and " +
                                this.maxBuffer,
                        });
                    }
                ...
                if (
                    !!feature.properties &&
                    !!feature.properties.geometryOperation
                ) {
                    ...
                    try {
                        ...
                        hint.push({
                            level: "warning",
                            message:
                                'Group operation be either "intersect" or "union"',
                        });
                    }
                }
                ...
                if (!!feature.properties && !!feature.properties.inverted) {
                    ...
                    try {
                        ...
                        hint.push({
                            level: "warning",
                            message:
                                'Property "inverted" must be the boolean "true" or "false" (no quotes)',
                        });
                    }
                }
            } finally {
        ...
        this.updateSearchResultsLayers = function () {
            if (
                ...
                self.filter.feature_collection()["features"].length > 0
            ) {
            ...
            _.each(aggregated.features, function (feature) {
                feature.properties.doc_count = _.reduce(
                    feature.properties.doc_count,
                    function (i, ii) {
                        return i + ii;
                    },
                    0,
        ...
        this.map.subscribe(function () {
            ...
            var filterUpdated = ko.computed(function () {
                return (
                    JSON.stringify(ko.toJS(this.filter.feature_collection())) +
                    this.filter.inverted()
                );
            }, this);
            ...
            this.searchAggregations.subscribe(
    ...
    */
        ...
        this.map().on("draw.update", function (e) {
            let updatedGeoms = _.filter(
                self.searchGeometries(),
                function (geom) {
    ...
    showDetailsFromFilter: function (resourceinstanceid) {
        const url =
            arches.urls.search_results + `?id=${resourceinstanceid}&tiles=true`;
    ...
    getMissingGraphAndShowReport: function (graphId, resource, searchResults) {
        ...
        let url =
            arches.urls.api_bulk_resource_report + `?graph_ids=${[graphId]}`;
        $.getJSON(url, function (resp) {
            var bulkResourceReportCache =
                searchResults.bulkResourceReportCache();
    ...
    updateFilter: function () {
        ...
        var useMaxBuffer = this.useMaxBuffer(
            this.bufferUnit(),
            this.buffer(),
            this.maxBuffer,
        );
    ...
    zoomToGeoJSON: function (data) {
        var mapData = data.properties.geometries.reduce(
            function (fc1, fc2) {
    ...
    updateQuery: function () {
        ...
        if (this.filter.feature_collection().features.length > 0) {
            if (
                this.getFilterByType("term-filter-type").hasTag(this.type) ===
                false
            ) {
            ...
            this.filter.feature_collection().features[0].properties[
                "inverted"
            ] = this.filter.inverted();
            queryObj[componentName] = ko.toJSON(
                this.filter.feature_collection(),
            );
            this.hasMultipleGeometries(
                this.filter.feature_collection().features.length > 1,
            );
        } else {
    ...
    restoreState: function () {
        ...
        this.geometryOperation = ko
            .observable(geometryOperation)
            .extend({ deferred: true });
        this.bufferUnit = ko.observable(bufferUnit).extend({ deferred: true });
        this.filter.inverted = ko
            .observable(inverted)
            .extend({ deferred: true });
        if (hasSpatialFilter) {
            this.getFilterByType("term-filter-type").addTag(
    ...
    updateResults: function () {
        if (!!this.searchResults.results) {
            this.searchAggregations({
                ...
                geo_aggs:
                    this.searchResults.results.aggregations.geo_aggs.inner
                        .buckets[0],
            });
    ...
    clear: function (reset_features) {
        ...
        this.getFilterByType("term-filter-type").removeTag(
            "Map Filter Enabled",
        );
        this.draw.deleteAll();
    ...
    fitToAggregationBounds: function (forceFitBounds) {
        ...
        if (agg && agg.geo_aggs.bounds.bounds && this.map()) {
            ...
            var bounds = [
                [aggBounds.top_left.lon, aggBounds.bottom_right.lat],
                [aggBounds.bottom_right.lon, aggBounds.top_left.lat],
            ];
            var maxZoom = ko.unwrap(this.maxZoom);
            maxZoom = maxZoom > 17 ? 17 : maxZoom;
            forceFitBounds =
                forceFitBounds == undefined
                    ? !this.pageLoaded
                    : forceFitBounds == true;
            this.mapFitBounds(
                bounds,
