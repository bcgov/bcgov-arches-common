...
var CommonSearchViewModel = function () {
    ...
    this.getFilter = function (filterName, unwrap = true) {
        if (unwrap) return ko.unwrap(this.searchFilterVms[filterName]);
        return this.searchFilterVms[filterName];
    };
    this.getFilterByType = function (type, unwrap = true) {
        const filter = this.searchFilterConfigs.find(
    ...
    this.searchViewFiltersLoaded = ko.computed(function () {
        let res = true;
        Object.entries(this.searchFilterVms).forEach(function ([
            componentName,
            filter,
        ]) {
...
var SearchView = BaseManagerView.extend({
    initialize: function (options) {
        ...
        this.viewModel.sharedStateObject.userCanEditResources =
            this.viewModel.userCanEditResources;
        this.viewModel.sharedStateObject.userCanReadResources =
            this.viewModel.userCanReadResources;
